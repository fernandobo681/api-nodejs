...
export async function getAllScheduleAppointments(req, res) {
  await ScheduleAppointmentSchema
    ...
    .then((scheduleAppointments) =>  {
      if(scheduleAppointments.length > 0) { 
        ...
        res.status(404).json({ success: false, message: 'No scheduleAppointments found' });
      }
    }
    )
...
export async function getScheduleAppointmentById(req, res) {
  const { id } = req.params;
  await ScheduleAppointmentSchema
    .findById(id)
...
export async function updateScheduleAppointmentById(req, res) {
  const { id } = req.params;
...
export async function deleteScheduleAppointmentById(req, res) {
  const { id } = req.params;
  await ScheduleAppointmentSchema
    .findOneAndDelete({ _id: id })
    .then((scheduleAppointment) => {
      scheduleAppointment ? res.status(201).json({ success: true, message: 'ScheduleAppointment deleted successfully', data: scheduleAppointment }) : res.status(404).json({ success: false, message: 'No scheduleAppointment found' });
    })
