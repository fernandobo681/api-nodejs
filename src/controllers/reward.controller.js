  ...
  await RewardSchema
    ...
    .then((rewards) =>  {
      if(rewards.length > 0) { 
        ...
        res.status(404).json({ success: false, message: 'No rewards found: ' + err.message });
      }
    }
    )
...
async function deleteRewardById(req, res) {
  ...
  await RewardSchema
    ...
    .then((reward) => {
      reward ? res.status(201).json({ success: true, message: 'Reward deleted successfully', data: reward }) : res.status(404).json({ success: false, message: 'No reward found: ' + err.message });
    })
